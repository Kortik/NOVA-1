...
#define TIME 20000
#define BTIME 10000
#define DEF_TIME  Lapic::freq_bus * 1000
#define DEF_TIME2  Lapic::freq_bus / 1000 * q
...
Sc::Sc (Pd *own, mword sel, Ec *e) : Kobject (SC, static_cast<Space_obj *>(own), sel, 0x1), ec (e), cpu (static_cast<unsigned>(sel)), prio (0), budget (BTIME*10), left (0)
{
    trace (0, "SC:%p created (PD:%p Kernel)", this, own);
}

Sc::Sc (Pd *own, mword sel, Ec *e, unsigned c, unsigned p, unsigned q) : Kobject (SC, static_cast<Space_obj *>(own), sel, 0x1), ec (e), cpu (c), prio (p), budget (BTIME), left (0)
{
